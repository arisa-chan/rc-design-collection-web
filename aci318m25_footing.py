...
class ACI318M25FootingDesign:
    ...
    def _run_opensees_analysis(self, geom: FootingGeometry, loads: FootingLoads, soil: SoilProperties,
                               mat_props: MaterialProperties, is_service: bool = False) -> Tuple[dict, List[str]]:
        notes = []
        ...
        for i in range(nx + 1):
            for j in range(ny + 1):
                ...
                ops.uniaxialMaterial('Elastic', spring_mat, k_spring)
        ...
        res_data = {
            'W': W, 'MXX': MXX, 'MYY': MYY, 'MXY': MXY, 'VX': VX, 'VY': VY, 'Q_SOIL': Q_SOIL
        }
        return res_data, notes

    def generate_contour_plots(self, geom: FootingGeometry, grid_data: dict, mat: MaterialProperties = None,
                               d_eff: float = None, keys_only: list = None) -> Dict[str, str]:
    ...
    def check_two_way_shear(self, geom: FootingGeometry, loads: FootingLoads, mat: MaterialProperties) -> Tuple[
        ...
        return ratio <= 1.0, ratio, Vu, phi_Vn

    def _generate_detailed_calcs(self, geom, loads, soil, mat, is_transient,
                                  qmax, qmin, bearing_limit, bearing_ok, net_qa,
                                  M_x_pos, M_x_neg, M_y_pos, M_y_neg, V_x, V_y,
                                  bx, sx, by, sy, tx, stx, ty, sty,
                                  d_eff, d_eff_top, As_min,
                                  one_way_ok, one_way_ratio, phi_vc,
                                  two_way_ok, two_way_ratio,
                                  fs_ot_x, fs_ot_y, overturning_ok, ot_limit,
                                  W_footing, W_soil, P_total, sliding_resistance, Ka):
        calcs = {}
        ...
        calcs['flexure_bot'] = (
            ...
            f"</ul>"
            f"<p>Effective depth: $d = h - c_{{clear}} - d_b = {geom.thickness} - {geom.cover} - 20 = {d_eff:.0f}$ mm</p>"
        ...
        calcs['flexure_top'] = (
            ...
            f"</ul>"
            f"<p>Effective depth (top): $d_{{top}} = h - c_{{clear}} - 2d_b = {geom.thickness} - {geom.cover} - 40 = {d_eff_top:.0f}$ mm</p>"
        ...
        bo = 2 * (geom.column_width + d_eff) + 2 * (geom.column_depth + d_eff)
        beta = max(geom.column_width, geom.column_depth) / min(geom.column_width, geom.column_depth)
        bearing_area = geom.length * geom.width / 1e6
        qu = loads.axial_force / bearing_area
        critical_area = (geom.column_width + d_eff) * (geom.column_depth + d_eff) / 1e6
        Vu = loads.axial_force - qu * critical_area
        vc1 = 0.17 * (1 + 2 / beta) * math.sqrt(mat.fc_prime)
        vc2 = 0.083 * (40 * d_eff / bo + 2) * math.sqrt(mat.fc_prime)
        vc3 = 0.33 * math.sqrt(mat.fc_prime)
        vc = min(vc1, vc2, vc3)
        phi_Vn = 0.75 * vc * bo * d_eff / 1000

        calcs['shear_2way'] = (
            f"<h3>Two-Way (Punching) Shear Check</h3>"
            f"<p>Critical perimeter at $d/2$ from column face:</p>"
            f"$$b_o = 2(c_1 + d) + 2(c_2 + d) = 2({geom.column_width} + {d_eff:.0f}) + 2({geom.column_depth} + {d_eff:.0f}) = {bo:.0f} \\text{{ mm}}$$"
            f"<p>Factored shear demand:</p>"
            f"$$V_u = P_u - q_u \\cdot A_{{critical}} = {loads.axial_force:.1f} - {qu:.2f} \\times {critical_area:.4f} = {Vu:.1f} \\text{{ kN}}$$"
            f"<p>Concrete shear strength (minimum of three expressions, ACI 318M-25 §22.6.5.2):</p>"
            ...
            f"$$\\phi V_n = \\phi \\cdot v_c \\cdot b_o \\cdot d = 0.75 \\times {vc:.3f} \\times {bo:.0f} \\times {d_eff:.0f} / 1000 = {phi_Vn:.1f} \\text{{ kN}}$$"
            f"<p><strong>Check:</strong> $V_u = {Vu:.1f} \\leq \\phi V_n = {phi_Vn:.1f}$ kN "
    ...
    def _select_rebar(self, As_req: float, width: float, thickness: float, preferred_bar: str = None) -> Tuple[str, float]:
        ...
        if preferred_bar and preferred_bar in bar_sizes:
            ...
            bar_sizes = [preferred_bar] + [b for i, b in enumerate(bar_sizes) if i != idx]
    ...
    def perform_complete_design(self, geom: FootingGeometry, loads: FootingLoads, soil: SoilProperties,
                                mat: MaterialProperties, is_transient: bool = False,
                                preferred_bottom_bar: str = "D16", preferred_top_bar: str = "D16") -> FootingAnalysisResult:
        ...
        net_qa = soil.bearing_capacity - soil_overburden - surcharge_total  # net allowable for column loads only
        ...
        srv_data, srv_notes = self._run_opensees_analysis(geom, loads, soil, mat, is_service=True)
        notes.extend(srv_notes)
        qmax_total = np.max(srv_data['Q_SOIL'])  # total soil pressure (column + soil + surcharge)
        ...
        bearing_limit = soil.bearing_capacity * 1.33 if is_transient else soil.bearing_capacity
        ...
        if is_transient:
            notes.append(f"ℹ️ Transient load combination active. Gross bearing limit = {bearing_limit:.1f} kPa (1.33× qa).")
        if not bearing_ok:
            notes.append(f"⚠️ Total bearing pressure ({qmax_total:.1f} kPa) exceeds gross allowable ({bearing_limit:.1f} kPa).")
        if qmin_total <= 0.1:
            notes.append("ℹ️ Footing is experiencing partial uplift (tension ignored in soil).")
        if soil_overburden > 0:
            notes.append(f"ℹ️ Soil overburden = {soil_overburden:.1f} kPa (γ={soil.unit_weight} kN/m³, D={soil.soil_depth} mm).")
        if surcharge_total > 0:
            notes.append(f"ℹ️ Surcharge = {surcharge_total:.1f} kPa (DL={loads.surcharge_dl}, LL={loads.surcharge_ll}).")

        # 2. Ultimate Analysis (Strength)
        ult_data, ult_notes = self._run_opensees_analysis(geom, loads, soil, mat, is_service=False)
        ...
        d_eff_top = geom.thickness - geom.cover - 20 - 20  # account for bottom bars below
        rho_min = 0.0018
        As_min = rho_min * 1000 * geom.thickness
        ...
        bx, sx = self._select_rebar(req_as(max(M_x_pos, 0)), 1000, geom.thickness, preferred_bottom_bar)
        by, sy = self._select_rebar(req_as(max(M_y_pos, 0)), 1000, geom.thickness, preferred_bottom_bar)
        ...
        def req_as_top(M):
            ...
            C = M * 1e6
            disc = B ** 2 - 4 * A * C
            if disc < 0: return As_min * 2
            return max((-B - math.sqrt(disc)) / (2 * A), As_min)

        if has_bend:
            tx, stx = self._select_rebar(req_as_top(max(M_x_neg, 0)), 1000, geom.thickness, preferred_top_bar)
            ty, sty = self._select_rebar(req_as_top(max(M_y_neg, 0)), 1000, geom.thickness, preferred_top_bar)
        else:
            tx, stx = "", 0.0
            ty, sty = "", 0.0

        # ── Shear Checks ──
        d_eff = max(d_eff, 100.0)
        L_m = geom.length / 1000.0
        B_m = geom.width / 1000.0
        d_m = d_eff / 1000.0
        col_w_m = geom.column_width / 1000.0
        ...
        two_way_ok, two_way_ratio, two_way_demand, two_way_cap = self.check_two_way_shear(geom, loads, mat)
        ...
        if not overturning_ok:
            if fs_ot_x < ot_limit:
                notes.append(f"⚠️ Overturning about X-axis inadequate. FS = {fs_ot_x:.2f} (min {ot_limit}).")
            if fs_ot_y < ot_limit:
                notes.append(f"⚠️ Overturning about Y-axis inadequate. FS = {fs_ot_y:.2f} (min {ot_limit}).")

        # Sliding Check
        sliding_resistance = P_total * math.tan(phi_rad)
        sliding_ok = sliding_resistance >= overturning_moment_x / (geom.thickness / 1000.0) if geom.thickness > 0 else True
        if soil.soil_depth > 0:
            notes.append(f"ℹ️ Sliding resistance = {sliding_resistance:.1f} kN (P_total × tan φ). Ka = {Ka:.3f}.")

        # Generate Visualizations
        d_eff = geom.thickness - geom.cover - 20
        plots = self.generate_contour_plots(geom, ult_data, mat, d_eff)
        srv_plots = self.generate_contour_plots(geom, srv_data, keys_only=['soil_pressure', 'settlement'])
        for k in ['soil_pressure', 'settlement']:
            if k in srv_plots:
                plots[k] = srv_plots[k]

        reinf = FootingReinforcement(bx, sx, by, sy, tx, stx, ty, sty,
                                     self.aci.calculate_development_length(bx, mat.fc_prime, mat.fy), 'D20', 600.0)

        # Overall DCR: use net pressure check and shear checks
        dcr = max(qmax_net / net_limit if net_limit > 0 else 0, one_way_ratio, two_way_ratio)

        detailed_calcs = self._generate_detailed_calcs(
            geom, loads, soil, mat, is_transient,
            qmax_total, qmin_total, bearing_limit, bearing_ok, net_qa,
            M_x_pos, M_x_neg, M_y_pos, M_y_neg, V_x, V_y,
            bx, sx, by, sy, tx, stx, ty, sty,
            geom.thickness - geom.cover - 20, geom.thickness - geom.cover - 40, As_min,
            one_way_ok, one_way_ratio, min(one_way_capacity_x, one_way_capacity_y),
            two_way_ok, two_way_ratio,
            fs_ot_x, fs_ot_y, overturning_ok, ot_limit,
            W_footing, W_soil, P_total, sliding_resistance, Ka
        )
        ...
        return FootingAnalysisResult(
            qmax_total, qmin_total, bearing_ok, one_way_ok, two_way_ok, reinf, dcr, list(set(notes)),
            max(M_x_pos, M_x_neg), max(M_y_pos, M_y_neg), V_x, V_y, plots,
            overturning_ok, fs_ot_x, fs_ot_y, bearing_limit, detailed_calcs,
            max(one_way_demand_x, one_way_demand_y), min(one_way_capacity_x, one_way_capacity_y),
            two_way_demand, two_way_cap
        )
    ...
    def calculate_qto(self, geom: FootingGeometry, res: FootingAnalysisResult) -> dict:
        ...
        items.extend(_optimize_cutting_stock(
            res.reinforcement.bottom_bars_x, res.reinforcement.bottom_spacing_x,
            geom.length, geom.width, "Bottom X Bars"))
        items.extend(_optimize_cutting_stock(
            res.reinforcement.bottom_bars_y, res.reinforcement.bottom_spacing_y,
            geom.width, geom.length, "Bottom Y Bars"))
        items.extend(_optimize_cutting_stock(
            res.reinforcement.top_bars_x, res.reinforcement.top_spacing_x,
            geom.length, geom.width, "Top X Bars"))
        items.extend(_optimize_cutting_stock(
            res.reinforcement.top_bars_y, res.reinforcement.top_spacing_y,
            geom.width, geom.length, "Top Y Bars"))
