...
def _footing_cache_key(data, geom, loads, soil, mat):
    payload = (
        data.length, data.width, data.thickness, data.cover,
        data.col_w, data.col_d, data.ecc_x, data.ecc_y,
        data.fc_prime, data.fy,
        data.soil_qa, data.soil_ks,
        data.soil_depth, data.soil_unit_weight, data.soil_friction_angle,
        data.surcharge_dl, data.surcharge_ll,
        data.pu_ult, data.mux_ult, data.muy_ult,
        data.pu_srv, data.mux_srv, data.muy_srv,
        data.bottom_bar_size, data.top_bar_size,
    )
...
def render_contour_selector(contours):
    ...
    """
    buttons = [
        air.Button(key_label, type="button",
        ...
        for key, key_label in CONTOUR_OPTIONS if key in contours
    ]
...
def render_progress_modal():
    ...
    return air.Div(
        ...
        air.Div(
            ...
            air.P("Depending on your computer specs, this may take about 2 to 10 seconds. Please wait.",
...
def _bar_options(selected):
    return [air.Option(db, value=db, selected=db == selected)
...
def setup_footing_routes(app):
            ...
            air.Main(
                air.Form(
                    ...
                    air.Div(
                        ...
                        air.Div(
                            ...
                            air.Div(air.Label("Thickness (mm)"),
                                    ...
                                    class_="form-group"),
                            air.Div(air.Label("Clear cover (mm)"),
                    ...
                    air.Div(
                        ...
                        air.Div(
                            air.Div(air.Label("Dimension along x (mm)"),
                                    air.Input(type="number", name="col_w", value=str(data.col_w)),
                                    class_="form-group"),
                            air.Div(air.Label("Dimension along y (mm)"),
                                    air.Input(type="number", name="col_d", value=str(data.col_d)),
                                    class_="form-group"),
                            air.Div(air.Label("Eccentricity along x (mm)"),
                                    air.Input(type="number", name="ecc_x", value=str(data.ecc_x)),
                                    class_="form-group"),
                            air.Div(air.Label("Eccentricity along y (mm)"),
                                    air.Input(type="number", name="ecc_y", value=str(data.ecc_y)),
                                    class_="form-group"),
                            class_="grid-2"
                        ), class_="card"
                    ),
                    ...
                    air.Div(
                        ...
                        air.Div(
                            ...
                            air.Div(air.Label("Bottom bar diameter"),
                                    air.Select(*_bar_options(data.bottom_bar_size),
                                               name="bottom_bar_size",
                                               style=BAR_SELECT_STYLE),
                                    class_="form-group"),
                            air.Div(air.Label("Top bar diameter"),
                                    air.Select(*_bar_options(data.top_bar_size),
                                               name="top_bar_size",
                                               style=BAR_SELECT_STYLE),
                                    class_="form-group"),
                            class_="grid-2"
                    ...
                    air.Div(
                        ...
                        air.Div(
                            air.Div(air.Label("Allowable soil bearing capacity (kPa)"),
                                    air.Input(type="number", name="soil_qa", value=str(data.soil_qa)),
                                    class_="form-group"),
                            air.Div(air.Label("Subgrade modulus (kN/m^3)"),
                                    air.Input(type="number", name="soil_ks", value=str(data.soil_ks)),
                                    class_="form-group"),
                            air.Div(air.Label("Depth of soil above footing (mm)"),
                                    air.Input(type="number", name="soil_depth", value=str(data.soil_depth)),
                                    class_="form-group"),
                            air.Div(air.Label("Soil unit weight (kN/m^3)"),
                                    air.Input(type="number", name="soil_unit_weight", value=str(data.soil_unit_weight)),
                                    class_="form-group"),
                            air.Div(air.Label("Soil friction angle (degrees)"),
                                    air.Input(type="number", name="soil_friction_angle", value=str(data.soil_friction_angle)),
                                    class_="form-group"),
                            air.Div(air.Label("Dead load surcharge (kPa)"),
                                    air.Input(type="number", name="surcharge_dl", value=str(data.surcharge_dl), step="any"),
                                    class_="form-group"),
                            air.Div(air.Label("Live load surcharge (kPa)"),
                                    air.Input(type="number", name="surcharge_ll", value=str(data.surcharge_ll), step="any"),
                                    class_="form-group"),
                            class_="grid-2"
                        ), class_="card"
                    ),
                    ...
                    air.Div(
                        ...
                        air.Div(
                            air.Div(
                                air.Input(type="checkbox", name="transient_loads", value="1",
                                          checked=data.transient_loads,
                                          style="width: auto; margin-right: 8px;"),
                                air.Span("Increase allowable bearing by 33% (wind/seismic governs)?",
    ...
    async def footing_design(request: air.Request):
        ...
        try:
            ...
            if (col_left < footing_left or col_right > footing_right or
                    col_front < footing_front or col_back > footing_back):
                modal_js = """
                ...
                return AirResponse(content=str(blueprint_layout(
                    ...
                    air.Div(
                        air.Div(
                            air.H2("Invalid Column Placement", style="margin-bottom: 8px; color: #DC2626;"),
                            air.P("The column extends beyond the footing boundary.",
                                  style="color: #6b7280; margin-bottom: 16px;"),
                            air.Div(
                                air.Ul(
                                    air.Li(air.Strong("Footing bounds along x:"),
                                           air.Span(f"{footing_left:.0f} to {footing_right:.0f} mm", class_="data-value")),
                                    air.Li(air.Strong("Column bounds along x:"),
                                           air.Span(f"{col_left:.0f} to {col_right:.0f} mm", class_="data-value")),
                                    air.Li(air.Strong("Footing bounds along y:"),
                                           air.Span(f"{footing_front:.0f} to {footing_back:.0f} mm", class_="data-value")),
                                    air.Li(air.Strong("Column bounds along y:"),
                                           air.Span(f"{col_front:.0f} to {col_back:.0f} mm", class_="data-value")),
                                ),
                                style="text-align: left; margin-bottom: 24px;"
                            ),
                            air.A("← Go Back", href="/footing", class_="button",
                    ...
                    air.Main(
                        air.Div(
                            air.H2("Invalid Column Placement", style="color: #DC2626;"),
                            air.P("The column extends beyond the footing boundary. Adjust the eccentricity or footing dimensions."),
                            air.A("← Go Back", href="/footing", class_="button secondary"),
                            class_="card"
                        )
                    )
                )), media_type="text/html")

            geom = FootingGeometry(data.length, data.width, data.thickness, data.cover, data.col_w, data.col_d,
                                   FootingType.ISOLATED, data.ecc_x, data.ecc_y, data.soil_ks)
            soil = SoilProperties(data.soil_qa, data.soil_unit_weight, data.soil_depth, data.soil_friction_angle)
            ...
            cached_res = None
            try:
                cache_key = _footing_cache_key(data, geom, loads, soil, mat) + ("_t" if is_transient else "_n")
            ...
            if cached_res is not None:
                ...
                res = engine.perform_complete_design(geom, loads, soil, mat, is_transient=is_transient,
            ...
            net_qa = soil.bearing_capacity - soil.unit_weight * (soil.soil_depth / 1000.0) - (loads.surcharge_dl + loads.surcharge_ll)
            ...
            report_content = air.Main(
                ...
                air.Div(
                    ...
                    air.Div(
                        air.Div(
                            air.H3("Project", style="font-size: 16px; margin-bottom: 8px; border:none; padding:0;"),
                            air.Ul(
                                air.Li(air.Strong("Project Name"), air.Span(data.proj_name, class_="data-value")),
                                air.Li(air.Strong("Location"), air.Span(data.proj_loc, class_="data-value")),
                                air.Li(air.Strong("Structural Engineer"), air.Span(data.proj_eng, class_="data-value")),
                                air.Li(air.Strong("Date"), air.Span(data.proj_date, class_="data-value")),
                            ), class_="section-box"
                        ),
                        air.Div(
                            air.H3("Footing & Column", style="font-size: 16px; margin-bottom: 8px; border:none; padding:0;"),
                            air.Ul(
                                air.Li(air.Strong("Footing"),
                                       air.Span(f"{data.length} mm × {data.width} mm × {data.thickness} mm", class_="data-value")),
                                air.Li(air.Strong("Column"),
                                       air.Span(f"{data.col_w} mm × {data.col_d} mm", class_="data-value")),
                                air.Li(air.Strong("Footing clear cover"),
                                       air.Span(f"{data.cover} mm", class_="data-value")),
                                air.Li(air.Strong("Column eccentricity"),
                                       air.Span(f"ex = {data.ecc_x} mm, ey = {data.ecc_y} mm", class_="data-value")),
                            ), class_="section-box"
                        ),
                        class_="grid-2"
                    ),
                    air.Div(
                        air.Div(
                            air.H3("Materials", style="font-size: 16px; margin-bottom: 8px; border:none; padding:0;"),
                            air.Ul(
                                air.Li(air.Strong("Concrete"), air.Span(f"f'c = {data.fc_prime} MPa", class_="data-value")),
                                air.Li(air.Strong("Rebar"), air.Span(f"fy = {data.fy} MPa", class_="data-value")),
                                air.Li(air.Strong("Bottom bars"), air.Span(data.bottom_bar_size, class_="data-value")),
                                air.Li(air.Strong("Top bars"), air.Span(data.top_bar_size, class_="data-value")),
                            ), class_="section-box"
                        ),
                        air.Div(
                            air.H3("Soil", style="font-size: 16px; margin-bottom: 8px; border:none; padding:0;"),
                            air.Ul(
                                air.Li(air.Strong("SBC"), air.Span(f"{data.soil_qa} kPa", class_="data-value")),
                                air.Li(air.Strong("Subgrade modulus"), air.Span(f"{data.soil_ks} kN/m^3", class_="data-value")),
                                air.Li(air.Strong("Unit weight"),
                                       air.Span(f"D = {data.soil_depth} mm, γ = {data.soil_unit_weight} kN/m^3", class_="data-value")),
                                air.Li(air.Strong("Friction angle"), air.Span(f"{data.soil_friction_angle}°", class_="data-value")),
                            ), class_="section-box"
                        ),
                        class_="grid-2"
                    ),
                    air.Div(
                        air.Div(
                            air.H3("Factored Loads", style="font-size: 16px; margin-bottom: 8px; border:none; padding:0;"),
                            air.Ul(
                                air.Li(air.Strong("Pu"), air.Span(f"{data.pu_ult} kN", class_="data-value")),
                                air.Li(air.Strong("Mux / Muy"),
                                       air.Span(f"{data.mux_ult} / {data.muy_ult} kN-m", class_="data-value")),
                            ), class_="section-box"
                        ),
                        air.Div(
                            air.H3("Service Loads", style="font-size: 16px; margin-bottom: 8px; border:none; padding:0;"),
                            air.Ul(
                                air.Li(air.Strong("Ps"), air.Span(f"{data.pu_srv} kN", class_="data-value")),
                                air.Li(air.Strong("Msx / Msy"),
                                       air.Span(f"{data.mux_srv} / {data.muy_srv} kN-m", class_="data-value")),
                            ), class_="section-box"
                        ),
                        class_="grid-2"
                    ),
                ...
                air.Div(
                    ...
                    air.Div(
                        air.Div(
                            ...
                            air.Ul(
                                air.Li(
                                    ...
                                    _status_badge(
                                        ...
                                        res.bearing_pressure_max <= res.bearing_limit_used),
                                ),
                                air.Li(air.Strong("Minimum soil bearing pressure"),
                            ...
                            air.Ul(
                                air.Li(air.Strong("Bottom bars along x"), air.Span(reinf_str_x, class_="data-value",
                                                                        style="color: #2563eb; font-weight: bold;")),
                                air.Li(air.Strong("Bottom bars along y"), air.Span(reinf_str_y, class_="data-value",
                                                                        style="color: #db2777; font-weight: bold;")),
                                air.Li(air.Strong("Top bars along x"), air.Span(reinf_str_tx, class_="data-value",
                                                                     style="color: #2563eb; font-weight: bold;")),
                                air.Li(air.Strong("Top bars along y"), air.Span(reinf_str_ty, class_="data-value",
                                                                     style="color: #db2777; font-weight: bold;")),
                            ),
                ...
                air.Div(
                    air.Button("Show Calculations", type="button", id="toggleCalcsBtn",
                               onclick="toggleCalcs()",
                ...
                air.Div(
                    air.Div(
                        air.Div(
                            ...
                            air.Raw(f"$$q_{{a,net}} = q_a - \\gamma_{{soil}} \\cdot D = {soil.bearing_capacity:.0f} - {soil.unit_weight:.1f} \\times {soil.soil_depth/1000:.3f} = {net_qa:.1f} \\text{{ kPa}}$$"),
                            air.Ul(
                                air.Li(air.Strong("Maximum pressure (FEA)"),
                                       air.Span(f"q_max = {res.bearing_pressure_max:.1f} kPa", class_="data-value")),
                                air.Li(air.Strong("Minimum pressure (FEA)"),
                                       air.Span(f"q_min = {res.bearing_pressure_min:.1f} kPa", class_="data-value")),
                                air.Li(
                                    air.Strong("Check"),
                                    _status_badge(
                        ...
                        air.Div(
                            ...
                            air.Raw(f"$$A_s = \\frac{{-B - \\sqrt{{B^2 - 4AC}}}}{{2A}}, \\quad A = \\frac{{\\phi f_y^2}}{{1.7 f'_c}}, \\quad B = -\\phi f_y d, \\quad C = M_u \\times 10^6$$"),
                            air.Ul(
                                air.Li(air.Strong("M_x (positive)"),
                                       air.Span(f"{M_x_pos:.2f} kN-m/m", class_="data-value")),
                                air.Li(air.Strong("M_y (positive)"),
                                       air.Span(f"{M_y_pos:.2f} kN-m/m", class_="data-value")),
                                air.Li(air.Strong("Effective depth"),
                                       air.Span(f"d = {d_eff_calc:.0f} mm", class_="data-value")),
                                air.Li(air.Strong("Minimum reinforcement"),
                                       air.Span(f"A_s,min = {0.0018 * 1000 * geom.thickness:.0f} mm²/m", class_="data-value")),
                                air.Li(air.Strong("X-direction"),
                                       air.Span(reinf_str_x, class_="data-value",
                                                style="color: #2563eb; font-weight: bold;")),
                                air.Li(air.Strong("Y-direction"),
                                       air.Span(reinf_str_y, class_="data-value",
                                                style="color: #db2777; font-weight: bold;")),
                            ), class_="section-box"
                        ),
                        class_="grid-2"
                    ...
                    air.Div(
                        air.Div(
                            ...
                            air.Ul(
                                air.Li(air.Strong("M_x (negative)"),
                                       air.Span(f"{M_x_neg:.2f} kN-m/m", class_="data-value")),
                                air.Li(air.Strong("M_y (negative)"),
                                       air.Span(f"{M_y_neg:.2f} kN-m/m", class_="data-value")),
                                air.Li(air.Strong("Effective depth (top)"),
                                       air.Span(f"d_top = {geom.thickness - geom.cover - 40:.0f} mm", class_="data-value")),
                                air.Li(air.Strong("X-direction"),
                                       air.Span(reinf_str_tx if reinf_str_tx else "None", class_="data-value",
                                                style=f"color: {'#2563eb' if reinf_str_tx else '#9ca3af'}; font-weight: bold;")),
                                air.Li(air.Strong("Y-direction"),
                                       air.Span(reinf_str_ty if reinf_str_ty else "None", class_="data-value",
                                                style=f"color: {'#db2777' if reinf_str_ty else '#9ca3af'}; font-weight: bold;")),
                            ), class_="section-box"
                        ),
                        air.Div(
                            air.H3("One-Way Shear Check"),
                            air.Raw(f"$$\\phi V_c = \\phi \\cdot 0.17 \\sqrt{{f'_c}} \\cdot b \\cdot d = 0.75 \\times 0.17 \\times \\sqrt{{{mat.fc_prime}}} \\times 1000 \\times {d_eff_calc} / 1000 = {res.one_way_shear_capacity:.1f} \\text{{ kN/m}}$$"),
                            air.Ul(
                                air.Li(air.Strong("Shear demand"),
                                       air.Span(f"V_u = {res.one_way_shear_demand:.1f} kN/m", class_="data-value")),
                                air.Li(air.Strong("Shear capacity"),
                                       air.Span(f"φV_c = {res.one_way_shear_capacity:.1f} kN/m", class_="data-value")),
                                air.Li(
                                    air.Strong("Check"),
                                    _status_badge(
                                        f"{res.one_way_shear_demand:.1f} kN/m ÷ {res.one_way_shear_capacity:.1f} kN/m = {res.one_way_shear_demand / res.one_way_shear_capacity if res.one_way_shear_capacity > 0 else 99.9:.2f} ({'Pass' if res.one_way_shear_ok else 'Fail'})",
                    ...
                    air.Div(
                        air.Div(
                            ...
                            air.Raw(f"$$b_o = 2(c_1 + d) + 2(c_2 + d) = 2({geom.column_width} + {d_eff_calc:.0f}) + 2({geom.column_depth} + {d_eff_calc:.0f}) = {2*(geom.column_width+d_eff_calc)+2*(geom.column_depth+d_eff_calc):.0f} \\text{{ mm}}$$"),
                            air.Ul(
                                air.Li(air.Strong("Shear demand"),
                                       air.Span(f"V_u = {res.two_way_shear_demand:.1f} kN", class_="data-value")),
                                air.Li(air.Strong("Shear capacity"),
                                       air.Span(f"φV_n = {res.two_way_shear_capacity:.1f} kN", class_="data-value")),
                                air.Li(
                                    air.Strong("Check"),
                                    _status_badge(
                                        f"{res.two_way_shear_demand:.1f} kN ÷ {res.two_way_shear_capacity:.1f} kN = {res.two_way_shear_demand / res.two_way_shear_capacity if res.two_way_shear_capacity > 0 else 99.9:.2f} ({'Pass' if res.two_way_shear_ok else 'Fail'})",
                        ...
                        air.Div(
                            ...
                            air.Raw(f"$$FS_x = \\frac{{M_{{R,x}}}}{{M_{{O,x}}}} = \\frac{{{P_total * B_m / 2:.1f}}}{{{abs(loads.service_moment_x) + abs(loads.shear_x) * geom.thickness/1000:.1f}}} = {res.fs_overturning_x:.2f}$$"),
                            air.Ul(
                                air.Li(air.Strong("Total vertical load"),
                                       air.Span(f"P_total = {P_total:.1f} kN", class_="data-value")),
                                air.Li(air.Strong("FS overturning (x-axis)"),
                                       _status_badge(
                                ...
                                air.Li(air.Strong("FS overturning (y-axis)"),
                                       _status_badge(
                ...
                air.Div(
                    ...
                    air.Div(
                        air.Div(air.Div("CONCRETE", class_="metric-label"), air.Div(f"{qto['volume']:.2f} m³", class_="metric-value"), class_="metric-card concrete"),
                        air.Div(air.Div("FORMWORK", class_="metric-label"), air.Div(f"{qto['formwork']:.2f} m²", class_="metric-value"), class_="metric-card formwork"),
                        air.Div(air.Div("REBAR WEIGHT", class_="metric-label"), air.Div(f"{qto['weight']:.1f} kg", class_="metric-value"), class_="metric-card rebar"),
                        class_="grid-3"
                    ),
                    class_="card"
                ),
        ...
        btn.textContent = 'Show Calculations';
            ...
            resp = AirResponse(content=str(blueprint_layout(
                air.Header(air.A("← Edit Inputs", href="/footing", class_="back-link no-print"),
                           air.H1("RC Isolated Footing Designer"),
                           air.P("in accordance with ACI 318M-25", class_="subtitle"),
                           class_="module-header"), report_content, head_extra=katex_head)),
                               media_type="text/html")
